...
// ----------------------------------------------------------------------------------------
#include "../appMien58/data.cpp"
const std::string folder(MienData::folder_input);
const char** data = MienData::input;
const int datac = sizeof(MienData::input)/sizeof(char*);
namespace recogn {
	const char* argv[] = {
		"recogn",
		"/Users/zhongsifen/Work/Mien58/data/card/1.jpg",
...
int main_recogn(int _argc, char** _argv)
{
	...
	shape_predictor sp;
	deserialize(MienConst::_DAT_SP) >> sp;
	// And finally we load the DNN responsible for face recognition.
	anet_type net;
	deserialize(MienConst::_DAT_NET) >> net;
	
	std::vector<matrix<rgb_pixel>> faces;
	matrix<rgb_pixel> img;
	for (int i=0; i<datac; ++i) {
		load_image(img, folder + data[i]);
